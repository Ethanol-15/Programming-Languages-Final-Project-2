...
public class PaymentController {
    ...
    public String paymentSuccess(@RequestParam("paymentId") String paymentId,
        ...
        if (payment != null) {
            // Calculate purchased API calls based on the payment amount
            int purchasedCalls = calculatePurchasedCalls(payment);

            // Update RateLimitService with purchased API calls
            rateLimitService.incrementCallCountAfterPayment(purchasedCalls);

            // Generate HTML page with success message and a button to go back
            return "<html>" +
                    "<head>" +
                    ...
                    "<p class='details'>You have successfully purchased " + purchasedCalls + " API calls.</p>" +
                    "</body>" +
                    "</html>";
        } else {
